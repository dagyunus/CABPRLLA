 ...
 */
static float MODBUS_AdcToTemperatureC(uint16_t adc_raw)
{
    float voltage;
    float temp_c;
    ...
    voltage = ((float)adc_raw * ADC_REF_VOLTAGE) / ADC_MAX_COUNT;

    /*
     * 10 mV / °C sensör varsayımı:
...
/*
 * Float değerin bellekteki IEEE-754 32-bit karşılığını uint32_t olarak döndürür.
 *
 * Örnek:
 * 25.5f -> 0x41CC0000
 */
static uint32_t MODBUS_FloatToU32(float value)
{
    union
    {
        float f;
...
void MODBUS_UpdateTable(void)
{
    ...
    temp_c = MODBUS_AdcToTemperatureC(adc_raw);

    ModBus_Table.Structure.Temp_Value = MODBUS_FloatToU32(temp_c);
}
